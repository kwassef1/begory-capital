...
const fields: Field[] = [
    ...
    {
        label: "Closing costs + initial repairs ($)",
        id: "rc-closing",
        ph: "10000",
    },
...
export default function RentalCalculator() {
    ...
    function calculate() {
        ...
        const coc =
            cashInvested > 0
                ? (((noi - monthlyPI) * 12) / cashInvested) * 100
                : 0;
        setResults({
            loanAmount,
            monthlyPI,
                        ...
                        <div className="border-t border-border pt-4 flex flex-col gap-3">
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                {[
                                    ...
                                    {
                                        label: "Cash invested",
                                        value: fmt(results.cashInvested),
                                    },
                                    {
                            ...
                            <div className="flex items-center justify-between gap-4 border-t border-border pt-3">
                                ...
                                <Button variant="primary" asChild>
                                    <Link
                                        href={`/contact?deal=rental&purchase=${vals["rc-purchase"]}&down=${vals["rc-down"]}&rate=${vals["rc-rate"]}&rent=${vals["rc-rent"]}&cashFlow=${Math.round(results.cashFlow)}&capRate=${results.capRate.toFixed(1)}&coc=${results.coc.toFixed(1)}`}
                                    >
                                        Discuss this deal →
                                    </Link>
                                </Button>
